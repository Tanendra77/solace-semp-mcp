...
exports.createOAuthHandlers = createOAuthHandlers;
...
function verifyPkce(verifier, challenge) {
    try {
        const computed = crypto_1.default.createHash('sha256').update(verifier).digest('base64url');
...
function createOAuthHandlers(options) {
    ...
    setInterval(() => {
        ...
        for (const [k, v] of clients)
            if (now - v.createdAt > 24 * 60 * 60 * 1000)
                clients.delete(k);
    }, 300_000).unref();
    function mountRoutes(app) {
        ...
        app.post('/register', (req, res) => {
            ...
            if (!Array.isArray(redirect_uris) || redirect_uris.length === 0 ||
                !redirect_uris.every((u) => typeof u === 'string')) {
...
<form method="POST" action="/authorize/submit">
  ...
  <button type="submit">Authorize</button>
        ...
        app.post('/authorize/submit', express_1.default.urlencoded({ extended: false }), (req, res) => {
    ...
    function createMiddleware() {
        return (req, res, next) => {
            ...
            if (timingSafeEqual(bearerToken, apiKey))
                return next();
