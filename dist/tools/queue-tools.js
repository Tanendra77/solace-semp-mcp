...
async function handleListQueues(registry, brokerName, vpn, limit, offset) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/queues`, params: { count: limit } });
...
async function handleListQueueSubscriptions(registry, brokerName, vpn, queue, limit, offset) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/queues/${queue}/subscriptions`, params: { count: limit } });
    const data = result.data;
    let text = JSON.stringify(data, null, 2);
    const total = result.meta?.count;
    if (total !== undefined && total > limit)
...
async function handleListQueueMessages(registry, brokerName, vpn, queue, maxMessages, offset) {
    const broker = registry.getOrThrow(brokerName);
...
function registerQueueTools(server, registry) {
    server.tool('list_queues', 'List queues on a VPN. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), offset: zod_1.z.number().int().min(0).default(0) }, async ({ broker, vpn, limit, offset }) => ({ content: [{ type: 'text', text: await handleListQueues(registry, broker, vpn, limit, offset) }] }));
    ...
    server.tool('list_queue_subscriptions', 'Topic subscriptions on a queue. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), queue: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), offset: zod_1.z.number().int().min(0).default(0) }, async ({ broker, vpn, queue, limit, offset }) => ({ content: [{ type: 'text', text: await handleListQueueSubscriptions(registry, broker, vpn, queue, limit, offset) }] }));
