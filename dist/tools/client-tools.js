...
async function handleListClients(registry, brokerName, vpn, limit, cursor) {
    const broker = registry.getOrThrow(brokerName);
    const params = { count: limit };
    if (cursor)
        params['cursor'] = cursor;
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clients`, params });
...
async function handleListClientSubscriptions(registry, brokerName, vpn, clientName, limit, cursor) {
    const broker = registry.getOrThrow(brokerName);
    const params = { count: limit };
    if (cursor)
        params['cursor'] = cursor;
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clients/${clientName}/subscriptions`, params });
    const data = result.data;
    let text = JSON.stringify(data, null, 2);
    const next = nextCursor(result.meta?.paging?.nextPageUri);
    if (next)
...
async function handleListClientConnections(registry, brokerName, vpn, username, limit, cursor) {
    const broker = registry.getOrThrow(brokerName);
    const params = { count: limit };
    if (cursor)
        params['cursor'] = cursor;
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientUsernames/${username}/connections`, params });
    const data = result.data;
    let text = JSON.stringify(data, null, 2);
    const next = nextCursor(result.meta?.paging?.nextPageUri);
    if (next)
...
function registerClientTools(server, registry) {
    server.tool('list_clients', 'List connected clients on a VPN. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), cursor: zod_1.z.string().optional() }, async ({ broker, vpn, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClients(registry, broker, vpn, limit, cursor) }] }));
    ...
    server.tool('list_client_subscriptions', 'Topic subscriptions for a client. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), client_name: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), cursor: zod_1.z.string().optional() }, async ({ broker, vpn, client_name, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClientSubscriptions(registry, broker, vpn, client_name, limit, cursor) }] }));
    server.tool('list_client_connections', 'Connections for a client username. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), username: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), cursor: zod_1.z.string().optional() }, async ({ broker, vpn, username, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClientConnections(registry, broker, vpn, username, limit, cursor) }] }));
