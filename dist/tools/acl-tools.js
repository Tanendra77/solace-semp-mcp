...
async function handleListAclProfiles(registry, brokerName, vpn, limit, offset) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/aclProfiles`, params: { count: limit } });
...
async function handleGetAclProfile(registry, brokerName, vpn, name) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/aclProfiles/${name}` });
...
async function handleListClientUsernames(registry, brokerName, vpn, limit, offset) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientUsernames`, params: { count: limit } });
    const data = result.data;
    let text = JSON.stringify(data, null, 2);
    const total = result.meta?.count;
    if (total !== undefined && total > limit)
...
async function handleGetClientUsername(registry, brokerName, vpn, name) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientUsernames/${name}` });
    return JSON.stringify(result.data, null, 2);
}
...
async function handleListClientProfiles(registry, brokerName, vpn, limit, offset) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientProfiles`, params: { count: limit } });
    const data = result.data;
    let text = JSON.stringify(data, null, 2);
    const total = result.meta?.count;
    if (total !== undefined && total > limit)
...
async function handleGetClientProfile(registry, brokerName, vpn, name) {
    const broker = registry.getOrThrow(brokerName);
    const result = await new client_1.SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientProfiles/${name}` });
    return JSON.stringify(result.data, null, 2);
}
...
function registerAclTools(server, registry) {
    // ACL Profile READ tools
    server.tool('list_acl_profiles', 'List ACL profiles on a VPN. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), offset: zod_1.z.number().int().min(0).default(0) }, async ({ broker, vpn, limit, offset }) => ({ content: [{ type: 'text', text: await handleListAclProfiles(registry, broker, vpn, limit, offset) }] }));
    ...
    // Client Username READ tools
    server.tool('list_client_usernames', 'List client usernames on a VPN. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), offset: zod_1.z.number().int().min(0).default(0) }, async ({ broker, vpn, limit, offset }) => ({ content: [{ type: 'text', text: await handleListClientUsernames(registry, broker, vpn, limit, offset) }] }));
    ...
    // Client Profile READ tools
    server.tool('list_client_profiles', 'List client profiles on a VPN. Paginated.', { broker: zod_1.z.string(), vpn: zod_1.z.string(), limit: zod_1.z.number().int().min(1).max(500).default(50), offset: zod_1.z.number().int().min(0).default(0) }, async ({ broker, vpn, limit, offset }) => ({ content: [{ type: 'text', text: await handleListClientProfiles(registry, broker, vpn, limit, offset) }] }));
