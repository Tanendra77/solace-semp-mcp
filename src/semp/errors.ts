...
function extractDescription(error: AxiosError): string {
  try { return (error.response?.data as any)?.meta?.error?.description ?? error.message; }
  catch { return error.message; }
}

export function mapSempError(error: AxiosError, brokerName: string): string {
