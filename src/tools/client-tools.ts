...
export async function handleListClients(registry: BrokerRegistry, brokerName: string, vpn: string, limit: number, cursor?: string): Promise<string> {
  const broker = registry.getOrThrow(brokerName);
  const params: Record<string, string | number> = { count: limit };
  if (cursor) params['cursor'] = cursor;
  const result = await new SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clients`, params });
...
export async function handleListClientSubscriptions(registry: BrokerRegistry, brokerName: string, vpn: string, clientName: string, limit: number, cursor?: string): Promise<string> {
  const broker = registry.getOrThrow(brokerName);
  const params: Record<string, string | number> = { count: limit };
  if (cursor) params['cursor'] = cursor;
  const result = await new SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clients/${clientName}/subscriptions`, params });
  const data = result.data as unknown[];
  let text = JSON.stringify(data, null, 2);
  const next = nextCursor(result.meta?.paging?.nextPageUri);
  if (next) text += `\n\nMore results available. Use cursor="${next}" for the next page.`;
...
export async function handleListClientConnections(registry: BrokerRegistry, brokerName: string, vpn: string, username: string, limit: number, cursor?: string): Promise<string> {
  const broker = registry.getOrThrow(brokerName);
  const params: Record<string, string | number> = { count: limit };
  if (cursor) params['cursor'] = cursor;
  const result = await new SempClient(broker).request({ api: 'monitor', method: 'GET', path: `/msgVpns/${vpn}/clientUsernames/${username}/connections`, params });
  const data = result.data as unknown[];
  let text = JSON.stringify(data, null, 2);
  const next = nextCursor(result.meta?.paging?.nextPageUri);
  if (next) text += `\n\nMore results available. Use cursor="${next}" for the next page.`;
...
export function registerClientTools(server: McpServer, registry: BrokerRegistry): void {
  server.tool('list_clients', 'List connected clients on a VPN. Paginated.',
    ...
    async ({ broker, vpn, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClients(registry, broker, vpn, limit, cursor) }] }));
  ...
  server.tool('list_client_subscriptions', 'Topic subscriptions for a client. Paginated.',
    { broker: z.string(), vpn: z.string(), client_name: z.string(), limit: z.number().int().min(1).max(500).default(50), cursor: z.string().optional() },
    async ({ broker, vpn, client_name, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClientSubscriptions(registry, broker, vpn, client_name, limit, cursor) }] }));
  server.tool('list_client_connections', 'Connections for a client username. Paginated.',
    { broker: z.string(), vpn: z.string(), username: z.string(), limit: z.number().int().min(1).max(500).default(50), cursor: z.string().optional() },
    async ({ broker, vpn, username, limit, cursor }) => ({ content: [{ type: 'text', text: await handleListClientConnections(registry, broker, vpn, username, limit, cursor) }] }));
