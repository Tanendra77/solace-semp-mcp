...
export async function handleFindBackloggedQueues(registry: BrokerRegistry, brokerName: string, vpn: string, threshold: number): Promise<string> {
  ...
  const queues = result.data as Array<Record<string, unknown>>;
...
export function registerDiagnosticTools(server: McpServer, registry: BrokerRegistry): void {
  server.tool('find_backlogged_queues', 'Find queues exceeding a spool usage threshold. Useful for capacity planning.',
    { broker: z.string(), vpn: z.string(), spool_threshold_percent: z.number().int().min(0).max(100).default(80) },
    async ({ broker, vpn, spool_threshold_percent }) => ({ content: [{ type: 'text', text: await handleFindBackloggedQueues(registry, broker, vpn, spool_threshold_percent) }] }));
  server.tool('find_idle_consumers', 'Find queues with consumers that are not draining messages. Useful for diagnosing stuck consumers.',
    { broker: z.string(), vpn: z.string() },
    async ({ broker, vpn }) => ({ content: [{ type: 'text', text: await handleFindIdleConsumers(registry, broker, vpn) }] }));
  server.tool('detect_message_lag', 'Identify queues with message backlogs. Severity: HIGH (>1000), MEDIUM (>100), LOW (>0).',
    { broker: z.string(), vpn: z.string() },
    async ({ broker, vpn }) => ({ content: [{ type: 'text', text: await handleDetectMessageLag(registry, broker, vpn) }] }));
}
